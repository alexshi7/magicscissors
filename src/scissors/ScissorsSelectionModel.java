...
public class ScissorsSelectionModel extends SelectionModel {
    ...
    protected void appendToSelection(Point p) {
        ...
        PolyLine appendPolyLine = liveWire(p);
        selection.addLast(appendPolyLine);
        findPaths(vertexId);
    }
    ...
    public PolyLine liveWire(Point p) {
        ...
        //  Hint: The `ImageGraph` class provides methods that might help with steps 1 and 3.
        int vertexId = graph.idAt(p);
        List<Integer> path = pendingPaths.pathTo(vertexId);
    ...
    private class ShortestPathsWorker
        ...
        public PathfindingSnapshot doInBackground() {
            ...
            int counter = 0;
            int percentage = 0;
            while (counter < pathfinder.vertexCount()){
                PathfindingSnapshot snapshot = pathfinder.extendSearch(2500);
                percentage = (100*pathfinder.settledCount()/(pathfinder.vertexCount()));
                setProgress(percentage);
                System.out.println(percentage);
                publish(snapshot);
                counter+=2500;
                if (pathfinder.allPathsFound()){
                    return snapshot;
                }
                if (isCancelled()){
                    return snapshot;
        ...
        public void done() {
            ...
            try {
                PathfindingSnapshot finalresult = get();
                paths = finalresult;
