...
public abstract class SelectionModel {
    ...
    public int closestPoint(Point p, int maxDistanceSq) {
        ...
        int start = 0;
        for (PolyLine element:selection){
            Point counter = element.start();
            int counterx = counter.x-p.x;
            System.out.println(counterx);

            int countery = counter.y-p.y;
            System.out.println(countery);

            int distance = (counterx*counterx)+(countery*countery);
