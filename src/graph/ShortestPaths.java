...
public class ShortestPaths<VertexType extends Vertex<EdgeType>, EdgeType extends Edge> {
    ...
    public PathfindingSnapshot extendSearch(int maxToSettle) {
        ...
        while (settledcounter < maxToSettle && !allPathsFound()) {
            Integer vertexid = frontier.remove();
            settledIds.set(vertexid);
            settledcounter++;
            for (EdgeType e: graph.getVertex(vertexid).outgoingEdges()){
                int endId = e.endId();
                int weight = weigher.weight(e);
                int newDistance = distances[vertexid] + weight;

                if ((distances[endId] == -1 && endId != startId) || newDistance < distances[endId]) {
                    distances[endId] = newDistance;
                    predecessors[endId] = vertexid;
                    frontier.addOrUpdate(endId, newDistance);
                }
            }
